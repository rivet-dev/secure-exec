...
- standalone `NodeRuntime.exec()` headless CLI harnesses should pass `stdin: ""` when the tool expects EOF on non-TTY stdin; for Pi's direct `dist/main.js` print-mode path with an explicit `--print` message, set `process.stdin.isTTY = true` inside the sandbox harness to skip Pi's piped-stdin probe, otherwise the standalone path stalls before provider traffic
- standalone `NodeRuntime.exec()` benchmark or harness scripts that spend real time in promise-only library work (`pdfDoc.save()`, `zip.generateAsync()`, similar) need an explicit live handle such as a short `setInterval()` cleared in `finally`; otherwise exec-mode runs can exit `0` before the async workload emits its final stdout JSON
...
- module-load microbenchs that target transitive Pi package files should resolve them via `createRequire(realpathSync(PI_SDK_ENTRY)).resolve("pkg-or-file")`; `tsx` can reject `@mariozechner/pi-coding-agent/dist/index.js` package-export subpaths even though the real installed file exists
...
- per-scenario module-load `metrics.prom` files now include `secure_exec_v8_host_runtime_*` gauges for host-runtime RSS, heap usage, heap-limit utilization, and CPU totals; the generated summary/resource sections read those gauges directly, so `-` values usually mean the benchmark was run against stale built `@secure-exec/v8` output or a missing metrics scrape
...
- after the `DestroySessionResult` teardown ack work, the remaining warm fixed-overhead floor is single-digit milliseconds in current committed artifacts and is mostly `CreateSession -> InjectGlobals` plus a small residual bucket; do not rely on older `~109 ms` `ExecutionResult -> Destroy` notes without re-reading the committed summaries
- `session.destroy()` now waits on the runtime's explicit `DestroySessionResult` ack instead of a per-session `Ping/Pong` barrier; when validating teardown wins, read `comparison.md` frame-byte deltas for `send:Ping`, `recv:Pong`, and `recv:DestroySessionResult` together, because any remaining `Ping/Pong` bytes usually come from the one-time warm-snapshot barrier rather than session teardown
...
- module-load `_loadPolyfill` target hotspot reporting must keep an explicit unattributed remainder bucket when older `ipc.ndjson` logs lack `bridgeTarget`; do not silently drop those calls from per-target rankings or comparisons
...
- if a heavy module-load control stage such as `pdf-lib-end-to-end` true cold start hits the default 45s orchestration timeout, rerun with `SECURE_EXEC_BENCH_STAGE_TIMEOUT_MS=60000` instead of weakening the workload; keep the override at or below 60s per the repo timeout policy
- if a late full-suite module-load refresh flakes only one scenario after most artifacts are already fresh, rerun the affected scenario directly and then regenerate top-level `summary.*` / `comparison.*` / `transport-rtt.*` with `SECURE_EXEC_BENCH_REUSE_RESULTS=1` against the preserved baseline instead of throwing away the whole run
...
- snapshot-preloaded builtin polyfill bundles must use the same normalized module list in both `createNodeV8Runtime()` and `createNodeRuntimeDriverFactory()`; if the warmup bridge source and per-execute bridge source diverge, warm fresh-session `Execute` calls miss the warmed snapshot path
...
- when hoisting `postRestoreScript` bootstrap work into snapshot-cached bridge code, preserve the original order: run `require`/console/bootstrap setup before timing-freeze hardening, and apply custom-global exposure after timing mitigation. Freezing `SharedArrayBuffer` too early makes later `require-setup` re-expose it and crash with `Cannot redefine property: SharedArrayBuffer`
...
- sandbox `setImmediate()` cannot be a thin alias for `setTimeout(0)`; JSZip/readable-stream style worker pipelines use `setImmediate` as a queue-drain primitive, and zero-delay timer semantics can turn that into a spin/hang instead of bounded deferred progress
...
- hot `__bd:*` dispatch wrappers should route through the dedicated `_bridgeDispatch` bridge global instead of string-encoded `_loadPolyfill("__bd:...")` shims; when adding or moving host bridge globals, keep the shared bridge contracts, `NODE_CUSTOM_GLOBAL_INVENTORY`, runtime-global typings, and the native V8 sync/async bridge registries in lockstep or the bridge-registry policy tests will catch the drift
- the native V8 BridgeResponse cache envelope can safely store any V8-serializable value, not just strings; when extending it beyond `_loadPolyfill`, key cache hits on the bridge method plus request args plus serialized payload so `_bridgeDispatch` `_loadFile`/`_loadFileSync` source envelopes can reuse cross-session refs without replaying the wrong transformed module body
...
- sync module-resolution referrer normalization must only strip real module file extensions (`.js`, `.mjs`, `.cjs`, `.ts`, `.tsx`, `.json`, `.node`, etc.); dotted package directories like `yaml-1.1/` are valid directories, and treating any dotted last path segment as a file path sends package-internal relative requires down the slow async fallback
