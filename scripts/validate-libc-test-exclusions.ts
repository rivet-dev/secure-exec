...
/**
 * Validates libc-test-exclusions.json for integrity.
 *
 * Checks:
 ...
 *
 * Usage: pnpm tsx scripts/validate-libc-test-exclusions.ts
 */
...
const LIBC_TEST_WASM_DIR = resolve(__dirname, '../native/wasmvm/c/build/libc-test');
...
for (const [key, entry] of Object.entries(exclusions)) {
  ...
  // expected-fail entries must have issue URL
...
if (allTests.length === 0) {
  console.log('Warning: No compiled libc-test WASM binaries found — skipping binary checks');
  console.log(`  Build them with: make -C native/wasmvm/c libc-test`);
}
...
console.log(`\nlibc-test Exclusion List Validation`);
