...
declare const _fs: {
	...
	rename: {
		applySyncPromise: (ctx: undefined, args: [string, string]) => void;
	};
};
...
const fdTable = new Map<
	number,
	{ path: string; flags: number; position: number }
>();
let nextFd = 3;
...
const fs = {
	...
	readFileSync(
		...
		const pathStr =
			typeof path === "number" ? fdTable.get(path)?.path : toPathString(path);
		if (!pathStr)
			throw createFsError("EBADF", "EBADF: bad file descriptor", "read");
		const encoding =
			typeof options === "string"
				? options
				: (options as { encoding?: BufferEncoding | null })?.encoding;
	...
	writeFileSync(
		...
		const pathStr =
			typeof file === "number" ? fdTable.get(file)?.path : toPathString(file);
		if (!pathStr)
	...
	renameSync(oldPath: PathLike, newPath: PathLike): void {
		_fs.rename.applySyncPromise(undefined, [
	...
	writeSync(
		...
		if (typeof buffer === "string") {
			...
			const length =
				(typeof lengthOrEncoding === "number" ? lengthOrEncoding : null) ??
	...
	readdir(
	...
	): Promise<string[] | Dirent[]> | undefined {
		if (typeof options === "function") {
			callback = options;
			options = undefined;
		}
		if (callback) {
			...
			return Promise.resolve(fs.readdirSync(path, options as ReaddirOptions));
		}
	},
	...
	realpathSync: Object.assign(
		function realpathSync(path: PathLike): string {
			...
			return (
				toPathString(path).replace(/\/\/+/g, "/").replace(/\/$/, "") || "/"
			);
		},
		{
	...
	createReadStream(
		...
		pipe: <T extends { write: (data: unknown) => void; end: () => void }>(
			dest: T,
		) => T;
	} {
...
type NodeFsMethodSignature<K extends keyof typeof nodeFs> =
	(typeof nodeFs)[K] extends (...args: infer A) => infer R
		? (...args: A) => R
		: (typeof nodeFs)[K];

// Validate key sync methods match Node.js signatures
type _CheckReadFileSync =
	typeof fs.readFileSync extends NodeFsMethodSignature<"readFileSync">
		? true
		: false;
type _CheckWriteFileSync =
	typeof fs.writeFileSync extends NodeFsMethodSignature<"writeFileSync">
		? true
		: false;
type _CheckStatSync =
	typeof fs.statSync extends NodeFsMethodSignature<"statSync"> ? true : false;
type _CheckExistsSync =
	typeof fs.existsSync extends NodeFsMethodSignature<"existsSync">
		? true
		: false;
