...
- the remaining temporary bootstrap shim set is now narrower: `internalBinding('buffer').setBufferPrototype()` still uses a scoped no-op, `internalBinding('trace_events').setTraceCategoryStateUpdateHandler()` stays a scoped no-op, and `internal/options` is still provided by a typed host shim for the eval-string entrypoint
...
- US-016 makes `module_wrap` and `contextify` explicit host-owned providers in the helper-child bootstrap runner instead of leaving them as implicit fallthroughs, and replaces the crash-prone `async_wrap.setupHooks()` path with a bounded JS-side provider that captures the bootstrap `nativeHooks` callbacks without calling the native assert path
- US-037 keeps that bounded bootstrap `async_wrap.setupHooks()` shim for `internal/bootstrap/node`, but now treats public `async_hooks` as an explicit helper-child replacement-runtime surface instead of a vague fallback: the runner serves a supported async_hooks module directly and rebinds post-bootstrap timer/task-queue callbacks before async eval proofs
- the supported async-hooks subset is intentionally narrow and documented: `AsyncLocalStorage` (`run()`, `enterWith()`, `exit()`, `disable()`, static `bind()` / `snapshot()`), `AsyncResource` (`constructor`, `bind()`, `runInAsyncScope()`, `emitDestroy()`, `asyncId()`, `triggerAsyncId()`), and `createHook()` callback delivery are covered across timers plus a supported `net` / `http` EventEmitter-listener path; it does this with helper-child scheduler/listener wrapping rather than claiming native `internal/async_hooks` parity
- still deferred or intentionally out of scope on this internal-only replacement runtime path: worker threads, inspector / trace-events integration, promises-specific hook parity beyond what host async hooks already provide incidentally, and any claim that the public `NodeExecutionDriver` / node-conformance runner has stopped using the older isolate/runtime stub
- US-027 promotes `process_methods` from a planned inventory entry to an explicit replacement-runtime provider in both the scaffold catalog and the helper-child bootstrap runner; the supported subset now deliberately covers bootstrap `patchProcessObject()`, `setEmitWarningSync()`, `resetStdioForTesting()`, `hrtime`/`cpuUsage`/`threadCpuUsage`/`memoryUsage`/`resourceUsage`, active-request/resource introspection, `_rawDebug()`, and host-owned process-state helpers such as `cwd()` / `chdir()` / `umask()`
- that provider intentionally blocks native-addon/process-replacement/debugger-control paths such as `dlopen()`, `execve()`, `abort()`, `_debugEnd()`, `_debugProcess()`, and `causeSegfault()` with deterministic unsupported errors instead of silently falling back to host native behavior
- `cares_wrap` and `modules` remain explicit planned entries rather than implicit gaps, while `fs`, `fs_dir`, and `fs_event_wrap` are marked deferred for the fs-first story with host-owned callback/wrapper/close semantics called out directly in the notes
...
- vendored `vm` first-light stays opt-in like other public builtin bring-up work: when the helper-child runner enables `--experimental-vm-modules`, it must patch `require('vm')` with `Module`, `SourceTextModule`, and `SyntheticModule` from vendored `internal/vm/module` the same way upstream `internal/process/pre_execution` does, or trivial `SourceTextModule` proofs silently fail even though `module_wrap` is present
