...
- relevant vendored node-conformance coverage is still blocked on file/module loading through the scoped replacement `node -e` path: vendored tests that do `require('../common')` reach `internal/modules/package_json_reader` and expect `internalBinding('modules').getNearestParentPackageJSON`, but the current bring-up runner is still eval-string-only rather than a full file-backed CommonJS loader, so no stale `expectations.json` entries were removed in this story
