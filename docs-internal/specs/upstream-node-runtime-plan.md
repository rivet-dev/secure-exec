...
- `process_methods`, `cares_wrap`, and `modules` remain explicit planned entries rather than implicit gaps, while `fs`, `fs_dir`, and `fs_event_wrap` are marked deferred for the fs-first story with host-owned callback/wrapper/close semantics called out directly in the notes
